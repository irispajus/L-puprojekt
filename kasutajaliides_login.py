...
class LoginWindow:
    ...
    def kasutajanimi_olemas_failis(self, kasutajanimi):
        try:
            df = pd.read_excel("kasutajad.xlsx")
    ...
    def edukas_sisselogimine(self):
        ...
        # Loob uue akna ja peamine aken sulgetakse
        uus_aken = tk.Tk()
        uus_aken.title("Tere tulemast")
        tk.Label(uus_aken, text="Edukas sisselogimine!").pack(pady=20)
        self.master.destroy()
        uus_aken.mainloop()
    ...
    def salvesta_excelisse(self, kasutajanimi, parool):
        ...
        df.to_excel("kasutajad.xlsx", index=False)
