...
class WelcomeWindow:
    ...
    # Display the next song in the sorted DataFrame
        if self.lugude_arv > 0:
            self.current_song = self.df.iloc[self.lugude_arv - 1]
            self.lugude_arv -= 1
    ...
    def like_action(self):
        ...
        # Sort and save the Excel file based on 'kaalupunktid'
        sort_and_save_excel(self.user_db_dir)
